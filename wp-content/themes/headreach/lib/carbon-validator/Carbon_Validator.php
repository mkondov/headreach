...
class Carbon_Validator {
	...
	public static function extend($rule_name, $callback) {
		if (!is_callable($callback)) {
			throw new Carbon_Validator_InvlalidSyntax("Second argument of " .
	...
	protected function get_rule_callback($rule_name) {
		...
		if (!method_exists($this, $method_name)) {
			throw new Carbon_Validator_InvlalidSyntax("Unknown validation rule: $rule_name");
		}
	...
	function validate_confirmed($value, $params) {
		if (!isset($params[0])) {
			throw new Carbon_Validator_InvlalidSyntax("You need to provide the name of the confirmation field.");
		}
	...
	function validate_file($value, $params) {
		...
		foreach ($allowed_formats as $format) {
			...
			if (!$mime_type) {
				throw new Carbon_Validator_InvlalidSyntax("Unknown format $format. Please add the mime type for this format " .
	...
	function validate_filesize($value, $params) {
		...
		if (preg_match('~^(\d+)([KM]?)$~i', $max_size, $matches)) {
			...
			throw new Carbon_Validator_InvlalidSyntax("Couldn't understand max upload size: $max_size");
		}
...
class Carbon_Validator_InvlalidSyntax extends Exception {}
